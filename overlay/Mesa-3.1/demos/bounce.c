...
#ifdef AMIX
  /*
   * AMIX can deliver the first VisibilityNotify before the callback is
   * registered, leaving the demo with a single rendered frame.  Seed the
   * idle loop so animation starts even if that initial event was missed.
   */
  glutIdleFunc(idle);
#endif
